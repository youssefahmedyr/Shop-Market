...
import {verifyResetCode} from '@/app/_api/auth'
import {z} from 'zod'
...
export default function VerifyCodePage() {
  ...
  const onSubmitCode: SubmitHandler<CodeFormData> = async (data) => {
    ...
    try {
      const res = await verifyResetCode(data.code)
      setMessage(res.message || res.status || 'Code verified successfully')
      ...
      <div className="w-full max-w-md bg-white border border-amber-100 rounded-2xl shadow-lg p-8 transform transition-all duration-300 hover:shadow-xl">
        <div className="text-center mb-8">
          <div className="inline-flex items-center justify-center w-16 h-16 bg-rose-50 rounded-2xl mb-4">
            <i className="fa-solid fa-key text-2xl text-rose-500"></i>
          </div>
          ...
          <p className="text-slate-500 text-sm">
            Check your email{' '}
            <span className="font-semibold text-rose-600">{email}</span> for the
            verification code
          </p>
        </div>
        ...
        <div className="mt-6 text-center">
          <button
            ...
            onClick={() =>
              router.push(
                `/auth/forgetPassword?email=${encodeURIComponent(email)}`
              )
            }
